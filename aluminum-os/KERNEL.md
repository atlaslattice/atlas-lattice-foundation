...
```
User ([owner])
  ├── Google Account: user@google-account.example
  ├── Microsoft Account: user@microsoft-account.example
  ├── Apple ID: user@icloud-account.example
  └── Devices
        ├── Pixel 9 (Android)
        ├── Pixel Watch
        ├── iPhone 16
...
> Built by [owner] Sheldon and the open-source community.
